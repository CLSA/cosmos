    ...
    foreach( $opal_view_class_name::select_objects() as $db_opal_view )
    {
      ...
      try 
      {
        ...
        if( $count > $db_opal_view->total )
        {
          ...
          log::info( sprintf( 'Found %s missing records in %s/%s', $new_interviews, $project_name, $view_name ) );
          $values = $opal_manager->get_all_values( $project_name, $view_name, $limit, $db_opal_view->total );
          ...
          $db_opal_view->total += count( $values );
          $db_opal_view->save();
        }
      }
      catch( \cenozo\exception\runtime $e )
      {
        log::info( $e->get_raw_message() );
      }
    }
