  ...
  public static function update_interview_list()
  {
    ...
    foreach( $study_phase_class_name::select_objects() as $db_study_phase )
    {
      ...
      foreach( $platform_class_name::select_objects() as $db_platform )
      {
        ...
        // set the view's date span
        $data = file_get_contents( sprintf( '%s/%s.json', OPAL_VIEWS_PATH, $opal_interview_view ) ); 
        $opal_manager->set_view( $project_name, $opal_interview_view, $data );

        // get interview data
        $values = $opal_manager->get_values( $project_name, $opal_interview_view );
        ...
        foreach( $db_study_phase->get_stage_type_list( $stage_type_sel ) as $stage_type )
        {
          ...
          $values = $opal_manager->get_values( $project_name, $opal_stage_view );
