...
$union_sql = array();
foreach($file_list[$rank] as $item)
{
  $union_sql[] =
   sprintf(
      '  ( '.
      ...
      '          substring_index( '.
      '            substring_index( '.
      '              qcdata, ",", %d ), ":", -1))/1048576.0,0) as fsz '.
      '    from interview i'.
      '    join stage s on i.id=s.interview_id'.
      '    where rank=%d'.
      '    and qcdata is not null'.
      '    and s.name="carotid_intima" '.
      '  ) ', $item, $rank );
}
...
if( $use_mode )
{
  ...
  $minsz = $res['minsz'];
  $maxsz = $res['maxsz'];
  $filesize_min = max(intval((($minsz + 0.5*($mode-$minsz))*1048576)),0);
  $filesize_max = intval(($mode + 0.5*($maxsz-$mode))*1048576);
}
else
{
  $avg=0;
  ...
  $res = $db->get_row( $sql );
  $stdev = $res['fstd'];
  $filesize_min = max(intval(($avg - $stdev_scale*$stdev)*1048576),0);
  $filesize_max = intval(($avg + $stdev_scale*$stdev)*1048576);
}
...
$file_left = array_slice($file_list[$rank], 0, count($file_list[$rank])/2);
$file_right = array_slice($file_list[$rank], count($file_list[$rank])/2);
...
$sql .= 'sum(if(qcdata is null, 0, if( ' .
         implode( ' and ', $and_sql ) . ',1,0))) as total_left_cineloop_only, ';
...
$sql .= 'sum(if(qcdata is null, 0, if( ' .
         implode( ' and ', $and_sql ) . ',1,0))) as total_right_cineloop_only, ';
...
$sql .= 'sum(if(qcdata is null, 0, if( ' .
         implode( ' and ', $and_sql ) . ',1,0))) as total_both_cineloop, ';
...
$percent_keys = array('total_left_cineloop_only','total_right_cineloop_only','total_both_cineloop','total_skip','total_unexplained_missing','total_contraindicated');
...
$page_heading = sprintf( 'CAROTID INTIMA CINELOOP RESULTS - Wave %d (%s - %s)',$rank,$begin_date,$end_date);
...
$page_explanation[]=sprintf('<li>presence of all left cineloop(s) only</li>');
$page_explanation[]=sprintf('<li>presence of all left cineloop(s) only</li>');
$page_explanation[]=sprintf('<li>presence of all right cineloop(s) only</li>');
$page_explanation[]=sprintf('<li>presence of all cineloop(s)</li>');
$page_explanation[]=sprintf('<li>presence of all right cineloop(s) only</li>');
$page_explanation[]=sprintf('<li>presence of all cineloop(s)</li>');
?>
