...
class carotid_intima_cineloop_generator extends table_generator
{
  ...
  protected function build_data()
  {
    ...
    else
    {
      ...
      $res = $db->get_row( $sql );
      $stdev = $res['fstd'];
      $filesize_min = max(intval(($avg - $this->standard_deviation_scale*$stdev)*1048576),0);
      $filesize_max = intval(($avg + $this->standard_deviation_scale*$stdev)*1048576);
    }
    ...
    $this->page_explanation[]=sprintf('presence of all left cineloop(s) only');
    $this->page_explanation[]=sprintf('presence of all left cineloop(s) only');
    $this->page_explanation[]=sprintf('presence of all right cineloop(s) only');
    $this->page_explanation[]=sprintf('presence of all cineloop(s)');
    $this->page_explanation[]=sprintf('presence of all right cineloop(s) only');
    $this->page_explanation[]=sprintf('presence of all cineloop(s)');
  }
...
$carotid_intima = new carotid_intima_cineloop_generator('carotid_intima', $rank, $begin_date, $end_date);

$qc_keys=array(
  'total_filesize_sub','total_filesize_par','total_filesize_sup'
);
$percent_keys = array('total_left_cineloop_only','total_right_cineloop_only',
...
$qc_keys[] = 'total_left_cineloop_only';
$qc_keys[] = 'total_right_cineloop_only';
$qc_keys[] = 'total_both_cineloop';
$carotid_intima->set_indicator_keys($qc_keys);

$carotid_intima->set_page_stage('CAROTID INTIMA CINELOOP');
