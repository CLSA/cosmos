...
class retinal_scan_generator extends table_generator
{
  ...
  protected function build_data()
  {
    ...
    if('mode' == $this->statistic)
    {
      ...
      if(1 == $this->rank)
      {
        $sql=sprintf(
          ...
          ') as t '.
          'where fsz>0 '.
          'group by fsz order by freq desc, fsz desc limit 1', $this->file_scale, $this->file_scale);
        ...
        $sql=sprintf(
          ...
          '  ) '.
          ') as t '.
          'where fsz>0', $this->file_scale, $this->file_scale);
    ...
    else
    {
      ...
      $sql .=
        'sum(if(qcdata is null, 0, '.
        'if(cast(trim("}" from substring_index(qcdata,":",-1)) as signed)>0,1,0))) as total_right_eye, ';
    }
