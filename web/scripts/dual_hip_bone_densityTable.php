...
$sql = sprintf(
  ...
  '    and s.name="dual_hip_bone_density" '.
  '  ) '.
  '  union all '.
  '  ( '.
  '    select '.
  ...
  ') as t '.
  'where fsz>0 '.
  'group by fsz order by freq desc, fsz desc limit 1', $rank, $rank);
...
$percent_keys = array('total_left_hip','total_right_hip','total_both_hip','total_skip','total_unexplained_missing','total_contraindicated');
...
$page_heading = sprintf( 'DEXA DUAL HIP RESULTS - Wave %d (%s - %s)',$rank,$begin_date,$end_date);
?>
