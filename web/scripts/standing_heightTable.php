...
$sql .= sprintf(
  ...
  'if(substring_index(substring_index(qcdata,",",1),":",-1)<%s,1,0))) as total_standing_height_sub, ', $height_dev_min);

$sql .= sprintf(
  'sum(if(qcdata is null, 0, '.
  'if(substring_index(substring_index(qcdata,",",1),":",-1) between %s and %s,1,0))) as total_standing_height_par, ',$height_dev_min,$height_dev_max);

$sql .= sprintf(
  'sum(if(qcdata is null, 0, '.
  'if(substring_index(substring_index(qcdata,",",1),":",-1)>%s,1,0))) as total_standing_height_sup, ',$height_dev_max);
