...
$sql .= 'sum(if(qcdata is null, 0, if(replace(replace(qcdata,"{freq_count:",""),"}","")<16,1,0))) as total_freq_sub, ';
$sql .= 'sum(if(qcdata is null, 0, if(replace(replace(qcdata,"{freq_count:",""),"}","")=16,1,0))) as total_freq_par, ';
...
$qc_keys = array('total_freq_sub','total_freq_par');
...
<html lang="en">
  ...
  <body>
    ...
    <ul>
      <?php
        echo "<li>frequency count sub: < 16 frequencies</li>";
        echo "<li>frequency count par: = 16 frequencies</li>";
      ?>
    </ul>
