...
class carotid_intima_still_generator extends table_generator
{
  ...
  protected function build_data()
  {
    ...
    $this->page_explanation[]=sprintf('presence of all left still(s) only');
    $this->page_explanation[]=sprintf('presence of all left still(s) only');
    $this->page_explanation[]=sprintf('presence of all right still(s) only');
    $this->page_explanation[]=sprintf('presence of all still(s)');
    $this->page_explanation[]=sprintf('presence of all right still(s) only');
    $this->page_explanation[]=sprintf('presence of all still(s)');
  }
...
$carotid_intima = new carotid_intima_still_generator('carotid_intima', $rank, $begin_date, $end_date);

$qc_keys=array(
  'total_filesize_sub','total_filesize_par','total_filesize_sup'
);
$percent_keys = array('total_left_still_only','total_right_still_only',
...
$qc_keys[] = 'total_left_still_only';
$qc_keys[] = 'total_right_still_only';
$qc_keys[] = 'total_both_still';
$carotid_intima->set_indicator_keys($qc_keys);

$carotid_intima->set_page_stage('CAROTID INTIMA STILL');
