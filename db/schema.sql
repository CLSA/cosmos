...
-- Thu 28 Mar 2019 05:17:58 PM EDT
...
-- -----------------------------------------------------
DROP TABLE IF EXISTS `cosmos`.`interview_has_indicator` ;
...
CREATE TABLE IF NOT EXISTS `cosmos`.`interview_has_indicator` (
  ...
  INDEX `fk_indicator_id` (`indicator_id` ASC),
  INDEX `fk_interview_id` (`interview_id` ASC),
  CONSTRAINT `fk_interview_has_indicator_interÿƒÚview_id`
    FOREIGN KEY (`interview_id`)
    REFERENCES `cosmos`.`interview` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
...
USE `cosmos`;
DROP procedure IF EXISTS `cosmos`.`update_reqn_current_reqn_version`;
