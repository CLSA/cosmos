...
-- Fri 13 Mar 2020 11:45:34 AM EDT
...
-- -----------------------------------------------------
DROP TABLE IF EXISTS `cosmos`.`platform` ;
...
-- -----------------------------------------------------
DROP TABLE IF EXISTS `cosmos`.`opal_view` ;
...
CREATE TABLE IF NOT EXISTS `cosmos`.`opal_view` (
  ...
  INDEX `fk_study_phase_id` (`study_phase_id` ASC),
  UNIQUE INDEX `uq_platform_id_study_phase_id` (`platform_id` ASC, `study_phase_id` ASC),
...
-- -----------------------------------------------------
DROP TABLE IF EXISTS `cosmos`.`outlier` ;
...
USE `cosmos`;
DROP procedure IF EXISTS `cosmos`.`update_outlier`;
...
USE `cosmos`;
DROP procedure IF EXISTS `cosmos`.`update_outliers`;
...
BEGIN
  ...
  SET @sql = CONCAT(
    ...
    "FROM indicator ",
    "JOIN stage_type ON indicator.stage_type_id = stage_type.id ",
    "JOIN opal_view ON stage_type.opal_view_id = opal_view.id ",
    "JOIN platform ON opal_view.platform_id = platform.id ",
    "JOIN ", @cenozo, ".study_phase ON opal_view.study_phase_id = study_phase.id ",
    "ORDER BY study_phase.code, platform.name, stage_type.name, indicator.name"
  );
  PREPARE statement FROM @sql;
  EXECUTE statement;
